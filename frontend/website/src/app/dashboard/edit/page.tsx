...
import { Button, Card, CardBody, CardFooter, Image, Input, Textarea } from "@heroui/react";
                ...
                {portfolio && (
                    <>
                        <div className="col-span-1 space-y-4">
                            <PortfolioStatusCard status={(portfolio as any)?.status ?? 0} url={portfolio?.url} />
                            <div className="bg-white dark:bg-[#0f0f0f] rounded-lg p-4 shadow">
                                <div className="text-sm text-gray-500 dark:text-gray-400">URL publique</div>
                                <div className="mt-2 text-lg font-medium">{portfolio?.url || "(non défini)"}</div>
                                <div className="mt-4">
                                    <Link href={`/${portfolio?.url || ""}`} className="text-blue-600">Prévisualiser</Link>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-gray-800">
                                ...
                                <div className="w-full flex flex-wrap items-center gap-3">
                                    {templatesStyles.map((style) => (
                                        <Button
                                            key={style.name}
                                            variant={currentColors.primary === style.primary ? "solid" : "bordered"}
                                            color={currentColors.primary === style.primary ? "primary" : "default"}
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                                    {Object.entries(currentColors as Record<string, string>).map(([key, value]) => (
                                        <ColorPicker
                                            key={key}
                                            colorKey={key}
                                            colorValue={value}
...
export const PortfolioStatusCard: React.FC<{ status?: number, url: string }> = ({ status, url }) => {
    return (
        <div className="bg-white dark:bg-[#0f0f0f] rounded-lg p-4 shadow">
            <div className="flex items-center justify-between">
                <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">Statut du portfolio</div>
                    <div className={`mt-2 text-lg font-semibold ${status == 1 ? 'text-green-500' : 'text-red-500'}`}>
                        {status == 1 ? 'En ligne' : 'Hors ligne'}
                    </div>
                </div>
                <div>
                    <div className={`w-3 h-3 rounded-full ${status == 1 ? 'bg-green-500' : 'bg-red-500'}`}></div>
                </div>
            </div>

            <div className="mt-4 flex items-center justify-between">
                <Link href={`/${url}`} className="text-blue-600">Voir</Link>
            </div>
        </div>
    );
}