...
const ModelViewer: React.FC<ModelViewerProps> = ({
  ...
  return (
    <div
      className="fixed -z-10 hidden lg:block transition-all duration-700 ease-in-out"
      style={{
        top: "5rem",
        right: "15%",
    ...
    >
      <Canvas style={{ width: "100%", height: "100%" }} camera={{ position: [0, 3, 8] }}>
        ...
        <Suspense>
          <Model modelPath="/model/Sae501_robot_complet_final.gltf" position={position} rotation={rotation} step={step} />
        </Suspense>
      </Canvas>
    </div>
  );
